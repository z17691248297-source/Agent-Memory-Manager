...
class MemoryViewRenderer:
    ...
    def render(self, state: TaskStateView, recent_turns: list[str], current_query: str, stage: str = "planning") -> str:
        ...
        parts = [
            ...
            "",
            "[Required Facts]",
            _bullet_lines(state.required_facts[:12]),
            "",
            "[Tool Key Findings]",
            _bullet_lines(state.tool_key_findings[:16]),
            "",
            "[Task State Facts]",
            _bullet_lines([_fact_line(item) for item in facts[:14]]),
            "",
            "[Required Answer Points]",
            _bullet_lines(state.required_answer_points[:12]),
            "",
            "Goals:",
    ...
    def _fit_token_budget(self, rendered: str, state: TaskStateView, recent_turns: list[str], current_query: str, stage: str) -> str:
        ...
        parts = [
            ...
            "",
            "[Required Facts]",
            _bullet_lines([_shorten(item, 120) for item in state.required_facts[:10]]),
            "",
            "[Tool Key Findings]",
            _bullet_lines([_shorten(item, 160) for item in state.tool_key_findings[:12]]),
            "",
            "[Task State Facts]",
            _bullet_lines([_shorten(_fact_line(item), 180) for item in facts]),
            "",
            "[Required Answer Points]",
            _bullet_lines([_shorten(item, 120) for item in state.required_answer_points[:10]]),
            "",
            "Goals:",
