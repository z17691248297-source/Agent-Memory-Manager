...
def test_long_session_event_sourced_benchmark_generates_csv_and_report(tmp_path, local_llm) -> None:
    results = tmp_path / "results"

    assert main(["benchmark", "--scenario", "long-session", "--output", str(results)]) == 0
...
def test_multi_stage_event_sourced_benchmark_generates_csv(tmp_path, local_llm) -> None:
    results = tmp_path / "results"

    assert main(["benchmark", "--scenario", "multi-stage", "--output", str(results)]) == 0
